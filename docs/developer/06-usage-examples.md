...
- [Example 15: Rotated Watermark](#example-15-rotated-watermark)
...
- [Example 22: VNSPDFGraphics Wrapper](#example-22-pdfgraphics-wrapper-vnspdfgraphicsutf)
...
```xojo
Dim pdf As New VNSPDFDocument()
// Note: constructor auto-adds page 1, so we're already on page 1
...
Dim currentPage As Integer = pdf.CurrentPage
MsgBox "Currently on page: " + Str(currentPage)  // Shows 3
...
// To points (using MillimetersToPoints helper or constant)
Dim widthPoints As Double = widthMm * VNSPDFModule.gkPointsPerMillimeter
// Result: ~595.28 points

// To inches
Dim widthInches As Double = widthMm / 25.4
// Result: ~8.27 inches
...
// VNSPDFModule.gkPointsPerMillimeter  = 2.83464567
// VNSPDFModule.gkPointsPerCentimeter  = 28.3464567
// VNSPDFModule.gkPointsPerInch        = 72
// VNSPDFModule.gkPointsToMM           = 0.352778
```
...
## Example 15: Rotated Watermark

This example demonstrates how to use transformation methods to create a rotated watermark using TransformBegin(), TransformRotate(), and TransformEnd().
...
pdf.SetAlpha(0.3, "Normal")      // Semi-transparent

// Calculate center of A4 page (210mm x 297mm)
Dim centerX As Double = 105.0
Dim centerY As Double = 148.5

// Begin transformation context
pdf.TransformBegin()

// Rotate 45 degrees counter-clockwise around center
pdf.TransformRotate(45, centerX, centerY)

// Draw watermark text (will be rotated)
pdf.Text(60, 150, "DRAFT")

// End transformation (restores graphics state)
pdf.TransformEnd()
...
pdf.TransformBegin()
pdf.TransformRotate(30, 50, 50)  // Rotate 30° around (50, 50)
pdf.Rect(40, 40, 20, 20, "DF")
pdf.TransformEnd()

// Example 3: Multiple rotations (nested contexts)
pdf.SetFont("Helvetica", "", 12)
pdf.SetTextColor(0, 0, 255)

For i As Integer = 0 To 11
  Dim angle As Double = i * 30  // Every 30 degrees

  pdf.TransformBegin()
  pdf.TransformRotate(angle, 105, 200)
  pdf.Text(105, 180, Str(i * 30) + "°")
  pdf.TransformEnd()
Next

// Save the document
If pdf.Ok() Then
  pdf.SaveToFile(SpecialFolder.Desktop.Child("rotation_example.pdf"))
Else
...
**Key Points**:
- Always pair `TransformBegin()` with `TransformEnd()`
- Transformations are applied within the context created by `TransformBegin()`
- The rotation angle is in degrees, counter-clockwise from 3 o'clock
- Graphics state (including position, colors, fonts) is preserved across transformations
- Multiple transformations can be nested for complex effects
- Common use case: Diagonal watermarks on documents
...
## Example 22: VNSPDFGraphics Wrapper

Demonstrates the VNSPDFGraphics wrapper class which provides a Xojo PDFGraphics-compatible API with full UTF-8 support.
...
// Basic drawing - same API as Xojo PDFGraphics
g.FontName = "Helvetica"
g.FontSize = 16
g.Bold = True
g.DrawingColor = &c000080
g.DrawText("PDFGraphics Wrapper Demo", 72, 72)

// UTF-8 text with unicode font
g.FontName = "unicode"
g.Bold = False
g.DrawText("中文: 你好世界", 72, 100)
g.DrawText("日本語: こんにちは世界", 72, 120)
g.DrawText("العربية: مرحبا بالعالم", 72, 140)

// Clipping - VNS has ClipEnd() unlike native Xojo
g.ClipToRectangle(72, 160, 150, 40)
g.DrawingColor = &c0000FF
g.DrawText("This long text is clipped to rectangle boundary", 75, 175)
g.ClipEnd()  // Restore clipping state

// Clip() function returns Self for method chaining
Dim clipped As VNSPDFGraphics = g.Clip(72, 220, 120, 50)
clipped.DrawingColor = &cFF0000
clipped.DrawText("Clipped region text", 75, 240)
g.ClipEnd()

// Polygon clipping with VNSPDFGraphicsPath
Dim trianglePath As New VNSPDFGraphicsPath
trianglePath.MoveToPoint(100, 300)
trianglePath.AddLineToPoint(175, 350)
trianglePath.AddLineToPoint(25, 350)
g.ClipToPath(trianglePath)
g.DrawingColor = &c008000
g.DrawText("Clipped to triangle", 50, 330)
g.ClipEnd()
...
g.DrawText("Rotated 45°", 0, 0)
g.RestoreState()

// Save PDF
pdf.SaveToFile(SpecialFolder.Desktop.Child("example22_wrapper.pdf"))
```

**Key Features**:
- **PDFGraphics-compatible API**: Same properties and methods as Xojo's PDFGraphics
- **UTF-8 Support**: Full Unicode text including CJK, Arabic, Hebrew
- **Clipping Methods**:
  - `ClipToRectangle(x, y, w, h)` - Rectangle clipping
  - `Clip(x, y, w, h)` - Returns Self for chaining (Xojo returns new Graphics)
  - `ClipToPath(GraphicsPath)` - Uses bounds rectangle
  - `ClipToPath(VNSPDFGraphicsPath)` - Full polygon clipping
  - `ClipEnd()` - Restores clipping state (not available in Xojo)
- **Transformations**: Rotate(), Translate(), Scale(), Transform()
- **State Management**: SaveState(), RestoreState()

**Xojo Clipping Limitations** (native PDFGraphics):
- `ClipToRectangle` has no undo - clip persists until page change
- `Clip()` returns new Graphics object with coordinates relative to clip origin
- `ClipToPath(GraphicsPath)` limited - GraphicsPath doesn't expose points

**VNSPDFGraphics Advantages**:
- `ClipEnd()` method to restore previous clipping state
- Polygon clipping via VNSPDFGraphicsPath
- Full UTF-8 text support including Arabic shaping
