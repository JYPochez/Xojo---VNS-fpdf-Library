...
**Module Flag**: `VNSPDFModule.hasPremiumFormsModule`
...
```xojo
VNSPDFModule.hasPremiumFormsModule = True
```
...
- **Push Buttons** (`PDFButton`): Clickable buttons with URL actions (JavaScript/URI)
...
- `PutCheckBox` - Checkboxes with `/Yes`/`/Off` appearance
- `PutRadioButtonGroup` + `PutRadioButtonWidget` - Grouped radio buttons
- `PutButton` - Push buttons with `/URI` actions
