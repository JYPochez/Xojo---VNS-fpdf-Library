...
| `Brush` | Variant | Gradient/pattern brush (LinearGradientBrush, RadialGradientBrush, PictureBrush, or Nil) |
| `Outline` | Boolean | Outline rendering (API compatibility - not rendered in PDF) |
| `FontUnit` | Integer | Always returns 0 (Points) - read-only |
| `MiterLimit` | Double | Miter limit for line joins (default 10.0) |
...
g.DrawRectangle(r As Rect)
g.FillRectangle(x, y, width, height)
g.FillRectangle(r As Rect)
g.ClearRectangle(x, y, width, height)       // Fill rectangle with white
g.ClearRectangle(r As Rect)
g.DrawRoundRectangle(x, y, width, height, cornerWidth, cornerHeight)
g.DrawRoundRectangle(r As Rect, cornerWidth, cornerHeight)
g.FillRoundRectangle(x, y, width, height, cornerWidth, cornerHeight)
g.FillRoundRectangle(r As Rect, cornerWidth, cornerHeight)
```
...
```xojo
g.DrawPicture(pic, x, y)                    // Draw at natural size
g.DrawPicture(pic, x, y, destWidth, destHeight, sourceX, sourceY, sourceWidth, sourceHeight)
```
...
path.Close
g.DrawPath(path)                             // GraphicsPath (uses bounds as fallback)
g.DrawPath(path As VNSPDFGraphicsPath)       // Full path rendering with curves/arcs
g.FillPath(path)                             // GraphicsPath (uses bounds as fallback)
g.FillPath(path As VNSPDFGraphicsPath)       // Full path rendering with curves/arcs
```

#### Object2D (Desktop/Web/Console only)

```xojo
// Draw any Object2D shape: RectShape, OvalShape, RoundRectShape, ArcShape,
// CurveShape, FigureShape, TextShape, PixmapShape, Group2D
g.DrawObject(obj As Object2D, deltaX As Double = 0, deltaY As Double = 0)
```
...
// Measure text block size without drawing
Dim sz As Size = g.TextBlockSize(text, maxWidth, maxHeight, alignment, truncateLastLine)
...
g.Rotate(angle)                              // Around current origin (stores for Tm text rendering)
g.Rotate(angle, centerX, centerY)            // Around a specific point
...
// Reset all state to defaults (font, colors, pen, line styles, brush)
g.ResetState()
```
...
```xojo
// Clip to rectangle (returns Self for chaining)
Dim clipped As VNSPDFGraphics = g.Clip(x, y, width, height)

// Clip to rectangle
g.ClipToRectangle(x, y, width, height)

// Clip to path
g.ClipToPath(path As GraphicsPath)
g.ClipToPath(path As VNSPDFGraphicsPath)
...
| Method | Description |
|--------|-------------|
| `ApplyFontSettings()` | Applies current font name, style (B/I/U), and size to the PDF document |
| `ApplyBrushToRect(x, y, width, height, inExternalClip)` | Applies gradient or pattern brush fill to a rectangular area |
| `ApplyBrushWithClip(clipProc, x, y, width, height)` | Reserved for generic clip-based brush application |
| `ApplyLineDash(values())` | Converts dash pattern values from points to mm and sets on PDF |
| `ApplyPictureBrushToRect(pb, x, y, width, height)` | Tiles a PictureBrush image to fill a rectangular area |
| `ContainsCJKCharacters(txt) As Boolean` | Checks if text contains CJK characters for character-based line breaking |
| `DrawTextWithRotation(str, x, y)` | Draws text using PDF Text Matrix (Tm) when rotation is pending |
| `HasBrush() As Boolean` | Returns True if a non-Nil brush is set |
| `SplitTextToLinesInternal(txt, maxWidthMM) As String()` | Word-wraps text into lines fitting maxWidth; supports CJK character-based wrapping |
| `TruncateWithEllipsis(txt, maxWidthMM) As String` | Truncates text with "..." ellipsis to fit within maxWidth |

**Object2D shape handlers (Desktop/Web/Console only):**

| Method | Description |
|--------|-------------|
| `DrawArcShapeObject(arc, deltaX, deltaY)` | Renders ArcShape with rotation and border settings |
| `DrawCurveShapeObject(curve, deltaX, deltaY)` | Renders CurveShape (quadratic/cubic Bezier or line) |
| `DrawFigureShapeObject(fig, deltaX, deltaY)` | Renders FigureShape (collection of CurveShapes forming a closed path) |
| `DrawGroup2DObject(group, deltaX, deltaY)` | Renders Group2D container with scale (rotation handled per-child) |
| `DrawOvalShapeObject(oval, deltaX, deltaY)` | Renders OvalShape with fill, border, and rotation |
| `DrawPixmapShapeObject(pix, deltaX, deltaY)` | Renders PixmapShape image with rotation |
| `DrawRectShapeObject(rect, deltaX, deltaY)` | Renders RectShape with fill, border, and rotation via raw PDF path commands |
| `DrawRoundRectShapeObject(rr, deltaX, deltaY)` | Renders RoundRectShape with corner radius, fill, border, and rotation |
| `DrawTextShapeObject(txt, deltaX, deltaY)` | Renders TextShape with font settings, alignment, and rotation |
