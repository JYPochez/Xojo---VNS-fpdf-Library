...
Premium modules use compile-time Boolean flags in VNSPDFModule:

```xojo
Public Const hasPremiumEncryptionModule As Boolean = False
Public Const hasPremiumZlibModule As Boolean = False
Public Const hasPremiumTableModule As Boolean = False
Public Const hasPremiumPDFAModule As Boolean = False
Public Const hasPremiumFormsModule As Boolean = False
```

When a premium module is installed, set the flag to True. The core library checks these flags at compile time to enable/disable premium features. This ensures no runtime overhead and clean separation between free and premium code.
