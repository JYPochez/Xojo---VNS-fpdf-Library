...
|--------|--------|----------|--------------|-----------------|
| **Encryption** | ✅ Complete | RC4-128, AES-128, AES-256 | RC4-40 only | RC4-40 + RC4-128 + AES-128 + AES-256 |
| **Digital Signatures** | ✅ Complete | PAdES-B-B, XAdES-BES | ❌ Not available | PDF + XML signing (part of Encryption module) |
| **Table** | ✅ Complete | SimpleTable, ImprovedTable, FancyTable | Manual Cell() calls | High-level API with auto pagination |
| **Zlib** | ✅ Complete | Pure Xojo deflate/inflate | Desktop/Web/Console only | All platforms including iOS |
| **E-Invoice** | ✅ Complete | Factur-X, ZUGFeRD, EN 16931 | ❌ Not available | CII XML + PDF/A-3b + conformity checker |
| **Barcodes** | ✅ Complete | QR, Code128 (free) + 8 more | QR + Code128 only | EAN-13, EAN-8, UPC-A, Code 39, ITF, Codabar, DataMatrix, PDF417 (part of E-Invoice module) |
| **HTML/Markdown** | ✅ Complete | LoadHTML(), LoadMarkdown() | ❌ Not available | HTML + Markdown to PDF with CSS support |
| **Forms** | 🔧 In Development | AcroForms (82% complete) | ❌ Not available | Interactive PDF forms |
...
├── Premium/
│   ├── EncryptionModule/
│   │   ├── VNSPDFEncryptionPremium.xojo_code   # RC4-128 + AES-128/256 encryption
│   │   ├── VNSAESConstants.xojo_code            # AES S-boxes and constants
│   │   ├── VNSAESCore.xojo_code                 # Pure Xojo AES-CBC engine
│   │   ├── VNSAESTables.xojo_code               # Pre-computed AES lookup tables
│   │   ├── VNSAESTest.xojo_code                 # AES test vectors
│   │   ├── VNSASN1.xojo_code                    # ASN.1 DER encoding (signatures)
│   │   ├── VNSX509Certificate.xojo_code         # X.509 certificate parsing
│   │   ├── VNSRSASigner.xojo_code               # RSA PKCS#1 v1.5 signing
│   │   ├── VNSPKCS7Signer.xojo_code             # PKCS#7/CMS signature generation
│   │   ├── VNSPDFSignature.xojo_code            # PAdES-B-B PDF signing
│   │   ├── VNSXMLCanonicalizer.xojo_code        # XML C14N canonicalization
│   │   └── VNSXAdESSigner.xojo_code             # XAdES-BES XML signing
│   ├── TableModule/
│   │   ├── VNSPDFTablePremium.xojo_code         # SimpleTable, ImprovedTable, FancyTable
│   │   ├── VNSPDFTableAccumulator.xojo_code     # Column accumulator for calculations
│   │   ├── VNSPDFTableColumnCalc.xojo_code      # Column calculation engine
│   │   ├── VNSPDFTableFooterConfig.xojo_code    # Footer configuration
│   │   ├── VNSPDFTableFooterStyle.xojo_code     # Footer styling
│   │   ├── VNSPDFManualTable.xojo_code          # Manual table builder API
│   │   ├── VNSPDFManualTableColumn.xojo_code    # Column definition
│   │   ├── VNSPDFManualTableHeaderCell.xojo_code # Header cell definition
│   │   └── VNSPDFManualTableFooterCell.xojo_code # Footer cell definition
│   ├── ZlibModule/
│   │   ├── VNSPDFZlibPremium.xojo_code          # Pure Xojo zlib orchestration
│   │   ├── VNSZlibPremiumDeflate.xojo_code      # Pure Xojo compression (RFC 1951)
│   │   ├── VNSZlibPremiumInflate.xojo_code      # Pure Xojo decompression
│   │   ├── VNSZlibPremiumAdler32.xojo_code      # Adler-32 checksum
│   │   ├── VNSZlibPremiumConstants.xojo_code    # Deflate constants
│   │   └── VNSZlibPremiumTrees.xojo_code        # Huffman tree generation
│   ├── PDFAModule/
│   │   └── VNSPDFPDFAPremium.xojo_code          # PDF/A output intents
│   ├── FormsModule/
│   │   └── VNSPDFFormsPremium.xojo_code         # AcroForms (82% complete)
│   ├── EInvoiceModule/
│   │   ├── VNSPDFEInvoicePremium.xojo_code      # E-Invoice orchestration
...
│   │   ├── VNSPDFEInvoiceXMLParser.xojo_code    # CII XML parsing (ReadEInvoice)
│   │   ├── VNSPDFEInvoiceValidator.xojo_code    # Profile-aware validation
│   │   ├── VNSPDFBarcode.xojo_code              # Barcode dispatcher
│   │   ├── VNSPDFBarcodeQR.xojo_code            # QR Code generator
│   │   ├── VNSPDFBarcodeCode128.xojo_code       # Code 128 generator
│   │   ├── VNSPDFBarcodeEAN.xojo_code           # EAN-13, EAN-8, UPC-A
│   │   ├── VNSPDFBarcodeCode39.xojo_code        # Code 39 generator
│   │   ├── VNSPDFBarcodeITF.xojo_code           # Interleaved 2 of 5
│   │   ├── VNSPDFBarcodeCodabar.xojo_code       # Codabar generator
│   │   ├── VNSPDFBarcodeDataMatrix.xojo_code    # DataMatrix generator
│   │   └── VNSPDFBarcodePDF417.xojo_code        # PDF417 generator
│   └── HTMLMarkdownModule/
│       ├── VNSPDFHTMLPremium.xojo_code          # LoadHTML/LoadMarkdown orchestration
│       ├── VNSPDFHTMLRenderer.xojo_code         # HTML-to-PDF rendering engine
│       ├── VNSPDFHTMLTableRenderer.xojo_code    # HTML table rendering
│       └── VNSPDFHTMLToken.xojo_code            # HTML token data class
├── Compression/
│   ├── VNSZlibModule.xojo_code                  # Main zlib interface (native-first)
│   └── VNSZlibModuleTest.xojo_code              # Zlib round-trip tests
└── VNSPDFModule.xojo_code                       # Enums, constants, utilities
```

### Design Principles

1. **Module Flags**: Boolean constants control feature availability
2. **Conditional Compilation**: `#If VNSPDFModule.hasPremiumXxxModule` gates premium code
...
Module availability is controlled by Boolean constants in **per-project constants files** (since v1.3). Each Xojo project has its own `_Premium_Constants.xojo_code` file:

| Project | Constants file |
|---------|---------------|
| Desktop | `Xojo_fpdf_Desktop/Xojo_fpdf_free_Premium_Constants.xojo_code` |
| Web | `Xojo_fpdf_Web/Xojo_fpdf_web_free_Premium_Constants.xojo_code` |
| Console | `Xojo_fpdf_console/xojo_consolepdf_free_Premium_Constants.xojo_code` |
| iOS | `Xojo_fpdf_ios/Xojo_iospdf_free_Premium_Constants.xojo_code` |

```xojo
// All 7 module flags (default: all False in free projects)
Public Const hasPremiumVNSEncryptionModule As Boolean = False  // RC4-128 + AES + Digital Signatures
...
Public Const hasPremiumVNSFormsModule As Boolean = False       // PDF AcroForms
Public Const hasPremiumVNSHTMLModule As Boolean = False        // HTML/Markdown import
Public Const hasPremiumVNSEInvoiceModule As Boolean = False    // E-Invoice + Barcodes
```
...
2. **Add the premium module files** to your project
3. **Set the corresponding flag to `True`** in your project's `_Premium_Constants.xojo_code` file
...
// To:
Public Const hasPremiumVNSEncryptionModule As Boolean = True
```

**Note**: Since v1.3, each project has its own constants file. Changing a flag for one project does not affect other projects sharing the same `PDF_Library/` folder.
...
// Save
pdf.Save(SpecialFolder.Desktop.Child("encrypted_rc4_128.pdf"))
```
...
**Location**: `PDF_Library/Premium/PDFAModule/VNSPDFPDFAPremium.xojo_code`
**Module Flag**: `hasPremiumVNSPDFAModule`
**Status**: 🔔 **Coming Soon** (Q2 2026) - Basic Output Intent works but still missing many features

### Features

The premium PDF/A module will provide support for archival-quality PDF documents:
...
// Save
pdf.Save(SpecialFolder.Desktop.Child("pdfa_document.pdf"))
```
...
**Location**: `PDF_Library/Premium/ZlibModule/VNSZlibPremiumDeflate.xojo_code` and `VNSZlibPremiumInflate.xojo_code`
...
When `hasPremiumVNSZlibModule = True`, iOS uses pure Xojo compression:
...
Module VNSPDFTablePremium
    ...
    Sub SimpleTable(doc As VNSPDFDocument, rs As RowSet, cellWidth As Double, _
                    cellHeight As Double, repeatHeaders As Boolean = True, _
                    footerConfig As VNSPDFTableFooterConfig = Nil)

    // Improved table with custom widths and auto alignment (✅ WORKING)
    Sub ImprovedTable(doc As VNSPDFDocument, rs As RowSet, columnWidths() As Double, _
                      cellHeight As Double, repeatHeaders As Boolean = True, _
                      footerConfig As VNSPDFTableFooterConfig = Nil)

    // Fancy table with professional styling (✅ WORKING)
    Sub FancyTable(doc As VNSPDFDocument, rs As RowSet, columnWidths() As Double, _
                   cellHeight As Double, repeatHeaders As Boolean = True, _
                   footerConfig As VNSPDFTableFooterConfig = Nil)
End Module
```
...
// Save
pdf.Save(SpecialFolder.Desktop.Child("table_example.pdf"))
```
...
- ✅ Footer rows (grand totals and intermediate subtotals)
- ✅ Column calculations (SUM, AVG, COUNT, MIN, MAX)
- ✅ VNSPDFTableFooterConfig for footer configuration
...
1. **Locate the premium module folder** (e.g., `EncryptionModule/`)
2. **Copy to** `PDF_Library/Premium/` folder (preserving subfolder structure)
3. **Add all files to your Xojo project**:
   - Open your Xojo project
   - Right-click on `PDF_Library/Premium` folder in Navigator
   - Select "Add File..."
   - Choose all `.xojo_code` files from the module subfolder

### Step 3: Enable Module Flag

1. **Open** your project's `_Premium_Constants.xojo_code` file
2. **Find the module flag**
...
| **Digital Signatures** | ✅ Complete | PAdES-B-B PDF, XAdES-BES XML, Adobe Acrobat validated |
| **Table** | ✅ Complete | SimpleTable, ImprovedTable, FancyTable, ManualTable, footers, calculations |
| **Zlib** | ✅ Complete | Pure Xojo deflate/inflate, iOS support, hash chain optimization |
| **E-Invoice** | ✅ Complete | Factur-X/ZUGFeRD EN 16931, CII XML, conformity checker |
| **Barcodes** | ✅ Complete | QR, Code128, EAN-13, EAN-8, UPC-A, Code 39, ITF, Codabar, DataMatrix, PDF417 |
| **HTML/Markdown** | ✅ Complete | LoadHTML, LoadMarkdown, block CSS, external images, merge fields |
| **Forms** | 🔧 82% complete | 9 control types, button actions, checkbox appearance |
| **PDF/A** | 🔔 Coming Soon | Output Intent, ICC profiles |
| **Total** | 7 of 9 complete | Forms and PDF/A in development |
...
```
MyApp/
├── MyApp_Free.xojo_project         # hasPremiumXxxModule = False
├── MyApp_Premium.xojo_project      # hasPremiumXxxModule = True
└── PDF_Library/
    ├── Core/
...
**Solutions**:
1. **Check module flag**: Verify `hasPremiumXxxModule = True` in your project's `_Premium_Constants.xojo_code` file
...
**Problem**: `VNSPDFModule.hasPremiumXxxModule` returns `False` despite setting to `True`
...
**Solutions**:
1. **Check Xojo version**: Ensure compatibility with Xojo 2025r3.1+
...
**VNS PDF Version**: 1.3
**Premium Modules**: Encryption + Signatures (✅), Table (✅), Zlib (✅), E-Invoice + Barcodes (✅), HTML/Markdown (✅), Forms (🔧 82%), PDF/A (🔔)
