...
**Digital Signature** - Enable automatic signing of exported invoices:
- Check **Digitally sign exported invoices** to activate
- Browse for an existing certificate and private key (.der files), OR click **Generate Self-Signed Certificate** to create one automatically using your company name and country
- Set a signing reason (e.g., "E-Invoice Signing") and location (optional)
- When enabled, every exported invoice is digitally signed with PAdES-B-B
...
The exported PDF includes a **Code128 barcode** encoding the invoice number for easy scanning and reference.
...
When you export an invoice, the PDF will be digitally signed. A green status label in the invoice window confirms "Will be signed with: [certificate name]" for new invoices, or shows the signing result after export.
...
*VNS E-Invoicer is built with the [VNS PDF Library](https://www.verynicesw.fr) for Xojo.*
